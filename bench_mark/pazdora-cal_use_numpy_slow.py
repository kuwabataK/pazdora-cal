...
def check_normal_drops(drops):
    ...
    if len(np.where(diff3 == 0)[0]) != 0:
        return False
    
    diff4 = np.roll(drops,1,axis=0) - drops
    diff5 = np.roll(drops,-1,axis=0) - drops
    diff6 = (100 * diff4[1:-1]) - diff5[1:-1]
    if len(np.where(diff6 == 0)[0]) != 0:
        return False
    return True
...
def generate_drops(height, width):
    return randint(1,7,(height,width))
...
def generate_field():
    ...
    return [drops.count(1),drops.count(2),drops.count(3),drops.count(4),drops.count(5),drops.count(6)]
